...
class System :
    ...
    def cancel_v2(self, organization_id):
        organization = self.get_organization(organization_id)
        # get list of users id in organization
        users_id = []
    ...
    def get_users_avatar(self, organization_id, user_id, access_to):
        users_avatar = []
        organization = self.get_organization(organization_id)
        admins_id = []
        # print(f'orgnization member{len(organization.admins)}')
        for admin in access_to:
            if admin.id != user_id:
                print(f'admin id : {admin.id} | user id: {user_id}')
                admins_id.append(admin.id)
        # print(f'admin id{len(admins_id)}')
        for acc in self.accounts_list:
            for user in acc.users_list:
                if user.id in admins_id:
                    users_avatar.append(acc.account_avatar)
        return users_avatar