...
acc3 = Account("march", "marchkung@gmail.com", "a")
...
project2 = Project(chat_h2_project, ElementCollector(), march, datetime.now(), project_detail2)
...
act1.add_accomplisher(tuatang1)
...
act2.add_accomplisher(tuatang1)
...
@app.delete("/session", tags=["Signout"])
...
@app.delete("/{organization_id}/account/cancellation", tags=["Organization"])
async def cancel_account(request:Request, response:Response, organization_id: int):
    system.cancel_v2(organization_id)
    # delete cookie
    cookies = request.cookies.keys()
    for cookie in cookies:
        response.delete_cookie(cookie)
...
@app.get("/{organization_id}/account/cancellation", response_class=HTMLResponse, tags=["Organization"])
async def show_cancellation(request: Request, organization_id: int):
...
async def view_home(request: Request, organization_id: int):
        if system.check_organization(organization_id) == True:
        ...
        is_admin, is_owner = system.check_user_access(user, organization.admins), system.check_user_access(user, organization.owners)
        return templates.TemplateResponse("home.html", {"request": request, "organization": organization, "account": account, "user": user, "user_projects": list_of_user_project, "user_chats": list_of_user_chat, "assignments": assignments, "events": events, "contacts": contacts, "todo_project_address": todo_project_address_list, "todo_organization_address": todo_organization_address_list, "event_project_address_list": event_project_address_list, "current_date": current_date, "events_data": events_data, "events": events, "organizations": organizations, "is_admin": is_admin, "is_owner": is_owner, "users": users})
...
def view_lineup(request: Request, organization_id: int):
    ...
    list_of_user_chat = organization.show_user_private_chat(user)
    contacts = system.get_contacts(user, organization.members)
    if (organization is None) or (not system.check_user_access(user, organization.members)):
...
def get_project(request: Request, organization_id: int, project_id: int):
    ...
    if (organization is None) or (project is None) or (not system.check_user_access(user, organization.members)) or (not system.check_user_access(user, project.members)):
        return RedirectResponse("/error")
...
def get_user_projects(organization_id: int, user_id: int):
    organization = system.get_organization(organization_id)
    user = system.get_user(user_id)
    if (organization is None) or (user is None) or (not system.check_user_access(user, organization.members)):
        return RedirectResponse("/error")
    list_of_user_project = organization.show_user_projects(user)
...
def add_chat(request: Request, organization_id: int, chat_history_id: int, data: dict):
    ...
    if (organization is None) or (chat_history is None) or (not system.check_user_access(user, chat_history.members)):
        return RedirectResponse("/error")
...
def use_adminland(request: Request, organization_id: int):
    ...
    user_id = int(request.cookies.get('user_id'))
    user = system.get_user(user_id)
    users_avatar = system.get_users_avatar(organization_id, user_id, organization.admins)
    owners_avatar = system.get_users_avatar(organization_id, user_id, organization.owners)
    print(type(users_avatar))
    for avatar in users_avatar:
        print(avatar)
    print(len(users_avatar))
    user_chats = organization.show_user_private_chat(user)
    contacts = system.get_contacts(user, organization.members)
    if (organization is None) or (not system.check_user_access(user, organization.members)):
        return RedirectResponse("/error")
    ...
    # print(account.account_avatar)
    return templates.TemplateResponse("adminland.html", {"request": request, "organization": organization, "user": user, "owners": owner_contacts, "admins": admin_contacts, "owner_permission": owner_permission, "admin_permission": admin_permission, "contacts": contacts, "user_chats": user_chats, "users_avatar": users_avatar, "account": account, "owners_avatar": owners_avatar})
...
def show_everyone(request: Request, organization_id: int):
    ...
    user_id = request.cookies.get('user_id')
    user = system.get_user(int(user_id))
    if (organization is None) or (not system.check_user_access(user, organization.members)) or (not organization.check_user_role(user, organization.admins)):
        return RedirectResponse("/error")
    user_chats = organization.show_user_private_chat(user)
...
def manage_access(request: Request, organization_id: int, user_id: int):
    ...
    if (organization is None) or (not system.check_user_access(visitor, organization.members)) or (not organization.check_user_role(visitor, organization.admins)):
        return RedirectResponse("/error")
...
def view_access_project(request: Request, organization_id: int):
    ...
    user_chats = organization.show_user_private_chat(user)
    contacts = system.get_contacts(user, organization.members)
    if (organization is None) or (not system.check_user_access(user, organization.members)) or (not organization.check_user_role(user, organization.owners)):
        return RedirectResponse("/error")
...
def view_todo_summary(request: Request, organization_id: int, user_id: int):
    ...
    contacts = system.get_contacts(user, organization.members)
    current_date = datetime.now()
    todo_project_address_list, todo_organization_address_list, todo_todo_topic_address_list = [], [], []
    count_checked = 0
    for todo in assignments:
        if todo.completed:
            count_checked += 1
        org ,project = system.find_element_address(todo, 2)
        todo_todo_topic_address_list.append
        todo_project_address_list.append(project)
        todo_organization_address_list.append(org)
...
def complete_todo(request: Request, organization_id: int, user_id: int, data: dict):
    ...
    for project in organization.projects:
        ...
        if isinstance(todo, ToDo):
            ...
            if todo.completed:
                notification = Notification(f"Completed: {todo.title}", datetime.now(), todo.added_by, todo.assigned_to, f"projects/{project.project_id}/todos/{todo_id}")
...
@app.get("/{organization_id}/projects/{project_id}/todos/{todo_id}/edit")
def edit_todos_topic(request: Request, organization_id: int, project_id: int, todo_id: int):
    organization, project = system.locater(organization_id, project_id)
    user_id = request.cookies.get('user_id')
    user = system.get_user(int(user_id))
    account = system.get_account_by_user_id(int(user_id))
    ...
    for user in organization.members:
        if user not in element.assigned_to:
            assignees.append(user)
    return templates.TemplateResponse('edit_todo.html', {'request': request, 'organization': organization, 'project': project, "account": account, "user": user, 'element': element, "contacts": contacts, "user_chats": user_chats, "assignees": assignees})

@app.put("/{organization_id}/projects/{project_id}/todos/{todo_id}/edit_todo")
def edit_todos_topic_detail(request: Request, organization_id: int, project_id: int, todo_id: int, data: dict):
    organization, project = system.locater(organization_id, project_id)
    user_id = request.cookies.get('user_id')
    user = system.get_user(int(user_id))
    element = project.element_collector.get_element(2, todo_id)
    project.element_collector.edit_element(2, todo_id, "title", data["title"])
    project.element_collector.edit_element(2, todo_id, "notify_who_when_done", data["notify_who_when_done"])
    project.element_collector.edit_element(2, todo_id, "start_date", data["start_date"])
    project.element_collector.edit_element(2, todo_id, "end_date", data["end_date"])
    project.element_collector.edit_element(2, todo_id, "content", data["content"])
    if data[f"assignee1"] != "":
        print(data["assignee1"])
        assignee_id = system.split_data(data["assignee1"], " : ", 1)
        assignee = system.get_user(int(assignee_id))
        element.add_assignee(assignee)
    return {"data": "success"}

@app.get("/{organization_id}/projects/{project_id}/{target}/{target_id}/remove_user/{user_id}")
def remove_target_user(organization_id: int, project_id: int, target: str, target_id: int, user_id: int):
    organization, project = system.locater(organization_id, project_id)
    user = system.get_user(user_id)
    if target == "todos":
        element = project.element_collector.get_element(ElementType.TODO.value, target_id)
        element.assigned_to.remove(user)
    elif target == "events":
        element = project.element_collector.get_element(ElementType.EVENT.value, target_id)
        element.participants.remove(user)
    return RedirectResponse(url=f"/{organization_id}/projects/{project.project_id}/{target}/{target_id}/edit", status_code=status.HTTP_303_SEE_OTHER)

@app.get("/{organization_id}/projects/{project_id}/{target}/{target_id}/delete")
def delete_element(organization_id: int, project_id: int, target: str, target_id: int):
    organization, project = system.locater(organization_id, project_id)
    if target == "todos":
        element = project.element_collector.get_element(ElementType.TODO.value, target_id)
        todos_topic = project.element_collector.get_element(ElementType.TODOSTOPIC.value, element.collector_id)
        project.element_collector.delete_element(ElementType.TODO.value, target_id)
        return RedirectResponse(url=f"/{organization_id}/projects/{project.project_id}/todos_topics/{todos_topic.id}", status_code=status.HTTP_303_SEE_OTHER)
    elif target == "events":
        element = project.element_collector.get_element(ElementType.EVENT.value, target_id)
        project.element_collector.delete_element(ElementType.EVENT.value, target_id)
        return RedirectResponse(url=f"/{organization_id}/projects/{project.project_id}/schedule", status_code=status.HTTP_303_SEE_OTHER)
    elif target == "todos_topics":
        element = project.element_collector.get_element(ElementType.TODOSTOPIC.value, target_id)
        todos = project.element_collector.todos
        for todo in element.get_todos(todos):
            project.element_collector.delete_element(ElementType.TODO.value, todo.id)
            del todo
        project.element_collector.delete_element(ElementType.TODOSTOPIC.value, target_id)
        return RedirectResponse(url=f"/{organization_id}/projects/{project.project_id}/todos_topics", status_code=status.HTTP_303_SEE_OTHER)

@app.get("/{organization_id}/projects/{project_id}/edit")
def edit_project_page(request: Request, organization_id: int, project_id: int):
    organization, project = system.locater(organization_id, project_id)
    user_id = request.cookies.get('user_id')
    user = system.get_user(int(user_id))
    account = system.get_account_by_user_id(int(user_id))
    ...
    if (organization is None) or (not system.check_user_access(user, organization.members)):
        return RedirectResponse("/error")
    return templates.TemplateResponse("edit_project.html", {"request": request, "organization": organization, "project": project, "account": account, "user": user, "contacts": contacts, "user_chats": user_chats})

@app.put("/{organization_id}/projects/{project_id}/edit_project")
def edit_project_page(request: Request, organization_id: int, project_id: int, data: dict):
    organization, project = system.locater(organization_id, project_id)
...
@app.delete("/{organization_id}/projects/{project_id}/messages/{message_id}/delete", response_class=HTMLResponse, tags=["Elements"])
async def delete_message(request: Request, organization_id: int, project_id: int, message_id: int):
    organization, project = system.locater(organization_id, project_id)
    project.element_collector.delete_element(ElementType.MESSAGE.value, message_id)
    return JSONResponse(status_code=200,content={"response": "success", "message_id" : message_id})
...
@app.delete("/{organization_id}/projects/{project_id}/document/{document_id}/delete", response_class=HTMLResponse, tags=["Elements"])
async def delete_document(request: Request, organization_id: int, project_id: int, document_id: int):
    organization, project = system.locater(organization_id, project_id)
    project.element_collector.delete_element(document_id//10000000, document_id)
