...
class ElementCollector:
    ...
    def collector_process(self, element_type: int, create_element: bool, element_detail: dict = None):
    ...
    def add_element(self, element_type: int, element_detail: dict):
        return_value = self.collector_process(element_type, True, element_detail)
    ...
    def get_element_collector(self, element_type: int):
        element_attribute = self.collector_process(element_type, False)[0]
    ...
    def delete_element(self, element_type: int, element_id: int):
        element = self.get_element(element_type, element_id)
        if element_type == ElementType.TODO.value:
            self.todos.remove(element)
        elif element_type == ElementType.EVENT.value:
            self.events.remove(element)
        elif element_type == ElementType.TODOSTOPIC.value:
            self.todos_topics.remove(element)
        elif element_type == ElementType.FOLDER.value:
            self.document_folders.remove(element)
            del element
        elif element_type == ElementType.DOCUMENT.value:
            self.document_files.remove(element)
            del element
        elif element_type == ElementType.MESSAGE.value:
            print("delete message")
            print(len(self.message_board))
            self.message_board.remove(element)
            print(len(self.message_board))
            del element
